...
public class SearchService {
    ...
    public Goods buildGoods(Spu spu) throws IOException {
        ...
        //根据cid1，cid2，cid3查询对于的分类名称
        List<String> names = this.categoryClient.queryNameByIds(Arrays.asList(spu.getCid1(), spu.getCid2(), spu.getCid3()));
        //根据spuId查询所有的sku
        List<Sku> skus = this.goodsClient.querySkuBySpuId(spu.getId());
        //初始化价格集合
        List<Double> prices = new ArrayList<>();
        //初始化一个skuMapList,每一个map相当于一个sku,map中的key取值：id title price image
        ...
        //查询spuDetail,目的是拿到genericSpec SpecialSppec
...
//        MatchQueryBuilder basicQuery = QueryBuilders.matchQuery("all", request.getKey()).operator(Operator.AND);
        ...
        List<Map<String,Object>> categories = getCategoriesAggResult(goodsPage.getAggregation(categoryAggName));
    ...
    private List<Map<String,Object>> getParamAggName(Long id, QueryBuilder basicQuery){
        //查询聚合的规格参数
        List<SpecParam> params = this.specClient.querySpecParam(null,id,null,true);
        //创建自定义查询构建器
        NativeSearchQueryBuilder queryBuilder = new NativeSearchQueryBuilder();
        //添加基本的查询条件
        queryBuilder.withQuery(basicQuery);
        ...
        //获取所有的规格参数聚合结果集Map<String, Aggregation>
    ...
    private List<Brand> getBrandAggResult(Aggregation aggregation) {
        LongTerms items = (LongTerms)aggregation;
        //获取桶集合
       return items.getBuckets().stream().map(bucket -> {
            Long id = bucket.getKeyAsNumber().longValue();
